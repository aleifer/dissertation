...
\qauthor{Ram\'on y Cajal, \citep{cajal_advice_1999}} 
\end{savequote}
...
The first term is the fundamental fluorophore distribution, the second term is thermal noise scaled by the local intensity of illumination, while the factor $q_b$ is a constant. This conditioned thermal noise term is zero-mean and gaussian to reasonable approximation, as will be verified later.
...
This equation has another interpretation, as well. Since $\sigma$ and $N$ are independent of $k$ and since the exponential is a monotonically increasing function of its argument, maximizing the likelihood across $k$ is equivalent to minimizing the energy of the difference between the observation and the reference across $k$ %DOUBLE CHECK!! DID we get the sign right here?
\begin{equation}
...
Left/right translations are handled by a circular convolution operation described in the following section.  Note that we are not accounting for differences in fluorescence $a_j$ caused by inefficient labeling during manufacturing,  because that would be too computationally intensive. For now we instead pre-screen barcodes to ensure that they are properly labeled, as discussed in Section  \ref{sec:thoeryResults}. As DNA origami manufacturing techniques improves over time, this step will become unnecessary. 

Once the bank of references is generated, it is then ready to be used in the multiple hypothesis test to determine the most likely barcode corresponding to a given observed signal, as described below. 
...
\subsection{Barcode Defects}\label{sec:defects}
...
Our model does not take into account stage drift that may cause slight misalignment between frames nor sparodic changes in laser intensity on a \textasciitilde1 minute time scale. Changes slower than \textasciitilde1 are corrected for by taking new regular background and blank images.
...
\section{Results} \label{sec:thoeryResults}
...
The software works very well at decoding barcodes that have been pre-screened to have uniform fluorescent labeling. While currently only a small percentage (\textasciitilde 15\%) of manufactured barcodes meet our criteria for uniform labeling, we are confident that in the coming years labeling efficiency will increase dramatically. One promising approaches is to add crosslinkers to the barcodes after they are fluorescently labeled to prevent fluorescent  DNA oligos from falling off.    
