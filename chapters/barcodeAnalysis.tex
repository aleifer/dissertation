...
To measure mean peak height, the software first identifies blobs in the binary mask $B_i$ that have an eccentricity and an area consistent with that of a single peak. It then finds the brightest pixel in each blob that matches the criteria ANDY: ARE THE CRITERIA IN THE TABLE ABOVE? and takes the arithmetic mean of all peaks in the image. 
...
\begin{equation}\label{eq:projectDown}
	   \mathbf{u}_i=u_i(x)=\frac{1}{2r\kappa\langle g_i \rangle}  \int_{y_min}^y_max{ {U_{\theta,i}(x,y)dy} },
\end{equation}
where $\langle g \rangle_i$ is the average peak height for this channel described in Section \ref{sec:normalizePeakHeight}, the rectangular region extends from $y_min$ to $y_max$,and $\kappa$ is a correction factor, the same for all channels, so that the average peak height remains unity even after the projection. $\kappa$ is the integral of the point spread function $h$ 
\begin{equation}
...
We must generate a bank of reference barcodes to test against the observed signal. Each reference barcode consists of three one-dimensional traces containing  gaussian peaks of height unity (Eq.~\ref{eq:psf}) superposed at the predicted location of the fluorescent spots, as shown for example in Fig.~FIGURE WITH PICTURE OF THREE TRACES. 
...
The bank of references are then used in the multiple hypothesis test to determine the most likely barcode corresponding to a given observed signal.


\subsection{Decoding the Barcode}
The main task of the software is to decode the barcode by identifying the most likely reference given an observed signal. The image processing prior to this section merely prepares the signals for decoding.  For a given observed signal, $\mathbf{u}$ composed of three one-dimensional traces as in Eq. 
\ref{eq:projectDown}, we invoke Eq.~\ref{eq:mle} to find the most likely reference.  We test all 10,368 references against the observed signal, 
...
\end{equation}
where $\mathbf{m}_{ki}$  is the the $k$th reference in the $i$th color channel, and $\sigma_i$ is the noise in the $i$th channel, as before. Note that $\lVert \mathbf{u}_i \rVert^2$  is independent of the reference $k$ and can be omitted from the minimization, 
\begin{equation}
...
\end{equation}
Our quest would be complete except that the barcodes in our observed signal may be translated from right to left within our window, as discussed in  Section \ref{sec:generateReferences}. Instead of adding  additional references to account for every possible translation for all of the 10,368 references we chose a computationally more efficient method of calculating the transpose $\mathbf{u}_i ^T \mathbf{m}_{ki}$ for all possible $x$ offsets between the reference and signal using a convolution, 
\begin{equation}
...
\Bigg\},
\end{equation}
where $*$ denotes convolution. We choose the $x$ offset that produces the largest product. This is computationally cheap, as we can implement the convolution using the Fast Fourier Transform. Moreover,  we need only take the  Fourier transform of each reference $m_k$ once ahead of time, and for each barcode we merely take the inverse transform of the product of the  Fourier transforms of the signal and the reference. 
...
For a given channel, the actual computation is as follows,
