...
We smooth the dark and background images and subtract and divide them, to remove the DC offset and compensate for the spatially varying illumination, respectively, according to
...
The first term is the signal of interest and the second term is the thermal noise scaled by the local illumination intensity, which we will show varies over the length of the image but is constant over the length of a barcode. The factor $q_b$ is a constant. The second thermal noise term is zero-mean, gaussian, and IID, as will be demonstrated later.
