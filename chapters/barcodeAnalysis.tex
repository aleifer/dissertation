...
%Explain the noise terms more fully.. what are they.. where are they coming from...
...
%ANDY: Add in sentance that the nouse terms are all zero mean, so n squigles are zero
...
The first term is the signal of interest  (qb is constant) and the second term is the thermal noise normalized by the illumination which we will show varies over the length scale of the image, but is constant over a barcode. Later we will show that the background of $U(\mathbf{r})$ is zero-mean, gaussian, and IID.
