...
Each blob ensemble can be examined to select those that represent a barcode. To qualify as a barcode, the area of a blob ensemble must lie in an acceptable range, its  eccentricity must be  $>0.75$,  and the major and minor axis of the ellipse that fits its second moment must lie in a range consistent with that of a barcode, see Table~\ref{table:morphCriteria}. The software also records the centroid, and angle of the major axis of the barcode, which will be used in Section \ref{sec:projectDown} to rotate and project down the barcode. 
...
\begin{equation}\label{eq:projectDown}
	   \mathbf{u}_i=u_i(x)=\frac{1}{2r\kappa\langle g_i \rangle}  \int_{y=-r}^r{ {U_{\theta,i}(x,y)dy} },
\end{equation}
