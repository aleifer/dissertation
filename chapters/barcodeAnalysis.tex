...
We would like to use the simple model described above to identify the barcodes in our two-dimensional images. Before we can do so, we must address the following discrepencies between assumptions in the simple model, and the realities of our barcode dataset:

	\begin{enumerate} %Why is this datest different from all other datasets?
	\item The simple model assumes that the noise is Gaussian, additive, zero-mean, and IID. But our images exhibit a slowly varying offset, and therefore exhibit noise that is neither zero-mean nor IID.
