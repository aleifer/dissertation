...
% Monday: since we'll talk about 1D case first, let's use 'u' for 1D then 'v' is natural for 2D data
% m_k = kth model, K = total # barcodes
% v(x,y) = raw observed voltage at pixel x,y. Probably don't mention the x,y coordinates for most of what we do
% c = camera offset constant
% n(x,y) = noise
% so v = a*m_k + n + c where a is ampl
% u(x) = 1D case, and also reduced observed data (offset removed and scale flattened)
...
%%%% Only got this far.......
%%%%
% Bayesian hypothesis testing: select model with largest inverse probability of correspondence to data u
\begin{equation}
