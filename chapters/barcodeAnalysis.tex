...
\end{equation}
Here $a_j$ is a factor that accounts for intensity variations due to inconsistent fluorescent labeling  during barcode assembly, and the star denotes the convolution operation. [WE NEED TO RETURN TO $a_j$ now that we have introduced it!]
...
Left/right translations are handled by a circular convolution operation described in the following section. Finally, the bank of references is then used in the multiple hypothesis test to determine the most likely barcode corresponding to a given observed signal, as described below.
